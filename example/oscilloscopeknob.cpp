...
OscilloscopeKnob::OscilloscopeKnob( const QString &title, double min, double max, QWidget *parent ):
    QWidget( parent )
{
...
QSize OscilloscopeKnob::sizeHint() const
{
...
void OscilloscopeKnob::setValue( double value )
{
    d_knob->setValue( value );
}

double OscilloscopeKnob::value() const
{
    return d_knob->value();
}

void OscilloscopeKnob::setTheme( const QColor &color )
{
    d_knob->setPalette( color );
}

QColor OscilloscopeKnob::theme() const
{
    return d_knob->palette().color( QPalette::Window );
}

void OscilloscopeKnob::resizeEvent( QResizeEvent *event )
{
